  ...
  return (
    <section className="relative overflow-hidden bg-black px-4 py-20 md:px-8" id="skills">
      <div className="pointer-events-none absolute inset-0 hidden md:block">
        <div className="absolute left-1/3 top-10 h-72 w-72 rounded-full bg-emerald-500/15 blur-[90px]" />
        <div className="absolute -bottom-16 right-10 h-80 w-80 rounded-full bg-lime-400/10 blur-[110px]" />
      </div>
        ...
        <div className="grid items-start gap-10 md:grid-cols-[minmax(0,1fr)_360px] md:items-center md:gap-4">
                ...
                <div className="grid grid-cols-3 gap-1.5 md:flex md:flex-wrap md:gap-10">
                  {items.map((skill) => {
                    ...
                    return (
                      <div
                        key={skill.name}
                        className="group flex aspect-square w-full max-w-22 flex-col items-center justify-center rounded-xl border border-emerald-300/20 bg-[#030904] p-1.5 shadow-lg shadow-black/25 backdrop-blur-sm transition-transform duration-300 hover:-translate-y-0.5 md:h-30 md:w-30 md:max-w-none md:flex-none md:bg-[#0d2818]/72 md:p-2"
                      >
                        {skill.isImage ? (
          ...
          <div className="mx-auto w-full max-w-96 pt-1 pr-6 sm:pr-3 md:pr-0 md:mx-0 md:justify-self-start lg:-ml-40">
            <div className="relative mx-auto h-96 w-105 origin-center -translate-x-8 scale-100 sm:-translate-x-6 sm:scale-110 md:translate-x-0 lg:scale-115">
              <div className="pointer-events-none absolute left-1/2 top-1/2 h-80 w-80 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#22c55e]/24 blur-[110px] md:bg-[#22c55e]/35" />
              <div className="pointer-events-none absolute left-1/2 top-1/2 h-64 w-64 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#4ade80]/17 blur-[85px] md:bg-[#4ade80]/25" />
              ...
              <div className="absolute left-1/2 top-1/2 h-30 w-30 -translate-x-1/2 -translate-y-1/2 rounded-full border border-emerald-300/65 bg-[#030904] shadow-[0_0_60px_rgba(34,197,94,0.6)] md:bg-[#0d2818]/94 md:shadow-[0_0_85px_rgba(34,197,94,0.9)]" />
              ...
              {clusterNodes.map((node) => {
                ...
                return (
                  <div
                    key={node.label}
                    className={`absolute ${node.pos} flex h-22 w-22 flex-col items-center justify-center border border-emerald-400/28 bg-[#030904] text-center shadow-md shadow-black/30 backdrop-blur-sm md:bg-[#0d2818]/82 [clip-path:polygon(25%_6%,75%_6%,100%_50%,75%_94%,25%_94%,0%_50%)]`}
                  >
                    {node.isImage ? (
